...
function addGamesToPage(games) {
  ...
  for (let i = 0; i < games.length; i++) {
    ...
    newDiv.innerHTML = `<img class="game-img" src="${game.img}" alt="${game.name}" /> <h3>${game.name}</h3> <p>${game.description}</p>`;
...
const totalFunded = GAMES_JSON.reduce( (acc, game) =>{
    return acc + game.pledged;
},0);
const numUnfunded = Unfunded.length;
// create a string that explains the number of unfunded games using the ternary operator
const description = `
  A total of $${totalFunded.toLocaleString()} has been raised for ${GAMES_JSON.length} games. 
...
firstGameElement.innerHTML = first.name;
firstGameContainer.appendChild(firstGameElement);
// do the same for the runner up item
const secondGameElement = document.createElement("p");
secondGameElement.innerHTML = second.name;
